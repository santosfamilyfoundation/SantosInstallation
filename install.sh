...
sudo apt-get update
sudo apt-get -y install git libopencv-dev ffmpeg python-numpy python-scipy python-matplotlib ipython ipython-notebook python-pandas python-sympy python-nose python-shapely python-skimage build-essential checkinstall cmake pkg-config yasm libtiff4-dev libjpeg-dev libjasper-dev libavcodec-dev libavformat-dev libswscale-dev libdc1394-22-dev libxine-dev libgstreamer0.10-dev libgstreamer-plugins-base0.10-dev libv4l-dev python-dev libtbb-dev libeigen3-dev libqt4-dev libgtk2.0-dev libfaac-dev libmp3lame-dev libopencore-amrnb-dev libopencore-amrwb-dev libtheora-dev libvorbis-dev libxvidcore-dev x264 v4l-utils libgtk2.0-dev libboost-all-dev doxygen unzip mercurial vim cmake-curses-gui sqlite3 sqlite3-doc libsqlite3-dev libcppunit-dev libcppunit-doc cmake-qt-gui python-sklearn
...
make doc
sudo make install
sudo echo "export PYTHONPATH="${PYTHONPATH}:/home/$USER/Traffic/trafficintelligence/python"" | sudo tee -a ~/.bashrc
sudo echo "export PYTHONPATH="${PYTHONPATH}:/home/vagrant/TrafficCloud"" | sudo tee -a ~/.bashrc
. ~/.bashrc
sudo ln /dev/null /dev/raw1394
cd
git clone https://github.com/santosfamilyfoundation/TrafficCloud.git
